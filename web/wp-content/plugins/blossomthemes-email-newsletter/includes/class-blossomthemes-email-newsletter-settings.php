		...
		switch ($platform) {
			...
			case 'aweber':
				...
				<div id="aweber" class="newsletter-settings<?php if($platform == 'aweber'){ echo ' current'; }?>">
					<div class="blossomthemes-email-newsletter-wrap-field">
						<label for="blossomthemes_email_newsletter_settings[aweber][app-id]"><?php _e('AWeber App ID : ','blossomthemes-email-newsletter');?></label> 
						<input type="text" id="blossomthemes_email_newsletter_settings[aweber][app-id]" name="blossomthemes_email_newsletter_settings[aweber][app-id]" value="<?php echo isset($blossomthemes_email_newsletter_settings['aweber']['app-id']) ? esc_attr( $blossomthemes_email_newsletter_settings['aweber']['app-id'] ): '';?>">  
					</div>
					<?php
					$blossomthemes_email_newsletter_settings = get_option( 'blossomthemes_email_newsletter_settings', true );
					$bten_aw_auth_info = get_option('bten_aw_auth_info');

					if(isset($blossomthemes_email_newsletter_settings['aweber']['app-id']) && ($blossomthemes_email_newsletter_settings['aweber']['app-id']!=''))
					{
						$appId = $blossomthemes_email_newsletter_settings['aweber']['app-id']; //Place APP ID here
					...
					{
						echo '<div class="blossomthemes-email-newsletter-note">' .sprintf( __( 'Save the AWeber APP ID to connect to the mailing lists in %1$sAWeber%2$s.', 'blossomthemes-email-newsletter' ), '<a href="https://www.aweber.com/index.htm" target="_blank">', '</a>').'</div>';
					}
				?>
				</div>
				<?php
